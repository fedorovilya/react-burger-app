...
export const OrderInfo = ({order}: Props) => {
	...
	const getStatusText = (order: FeedOrder) => {
		...
		} else return <p className={'pt-3 text text_type_main-small'} style={{color: "#00CCCC"}}>Выполнен</p>;
	}
			...
			return (
				<ul className={style.ingredients_flex}>
					{Array.from(ingredientCountMap).map(([id, count]) => {
						...
						return (
							<li className={style.ingredient_row}>
								<div className={style.imageWrapper} key={`order-${id}`}>
									<img className={style.imageWrapper_img} src={ingredient?.image}
										 alt={`ingredient-order-${id}`}/>
								</div>
								<p className={'text text_type_main-default'} style={{flex: 1}}>{ingredient?.name}</p>
	...
	return (
		<div className={style.flex}>
			<p className={"text text_type_digits-medium"} style={{alignSelf: 'center'}}>#{order.number}</p>
