...
import burgerConstructorSlice, {
	addIngredientToConstructor, clearList, initialState,
	removeIngredientFromConstructor,
	setBun, setIngredientsConstructorList
...
describe('тесты burgerConstructorSlice', () => {
	...
	it('должен успешно добавить ингредиент в конструктор', () => {
		const actual = burgerConstructorSlice(undefined, addIngredientToConstructor(ingredientMock));
		expect(actual.ingredients).toHaveLength(1);
		expect(actual.ingredients[0]).toEqual(ingredientMock);
		expect(actual.ingredients.find(ing => ing.item.type === 'bun')).toBeUndefined();
	});
	...
	it('должен успешно удалить ингредиент по id', () => {
		...
		const actual = burgerConstructorSlice(stateWithItems, removeIngredientFromConstructor(ingredientMock.id));
		expect(actual.ingredients).toHaveLength(1);
	...
	it('должен заменить список ингредиентов', () => {
		...
		const actual = burgerConstructorSlice(undefined, setIngredientsConstructorList(newIngredientsList));
