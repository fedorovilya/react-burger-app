...
export const initialState: UserData = {
	...
	error: null,
	isAuthorized: !!(
		localStorage.getItem('refreshToken') && Cookies.get('token')
	),
};
...
export const createRegisterRequest = createAsyncThunk<
	UserResponse,
	RegisterRequest
>('user/createRegisterRequest', async (registerData, thunkAPI) => {
	try {
		...
		if (!response.user || !response.accessToken || !response.refreshToken) {
			return thunkAPI.rejectWithValue(
				'Данные для успешной регистрации не получены в ответе'
			);
		}
...
export const createLoginRequest = createAsyncThunk<UserResponse, LoginRequest>(
	...
	async (loginData, thunkAPI) => {
		try {
			...
			if (!response.user || !response.accessToken || !response.refreshToken) {
				return thunkAPI.rejectWithValue(
					'Данные для успешной авторизации не получены в ответе'
				);
			}
...
export const createUpdateUserRequest = createAsyncThunk<
	UserResponse,
	UpdateUserRequest
>('user/createUpdateUserRequest', async (userData, thunkAPI) => {
	try {
